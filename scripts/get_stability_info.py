...
def metric_list_from_analyzer(analyzer, metric_name):
    quality_metrics = analyzer.get_extension('quality_metrics').get_data()
    qm_firing_rates = quality_metrics[metric_name]
    qm_firing_rates.name = str(start)
    return qm_firing_rates
...
end = datetime.strptime("2026-07-03 12:00:00", "%Y-%m-%d %H:%M:%S")
# 2026 07 06 12:00
...
for start, end in start_end_times:
    ...
    analyzer_path = Path(f'/ceph/scratch/chalcrow/fromgit/aeon_ss_playground/lupin_TM_si_output/{start.replace(" ", "T").replace(":", "-")}_{end.replace(" ", "T").replace(":", "-")}/shank_2/analyzer_merged')

    try:
        analyzer = si.load_sorting_analyzer(analyzer_path, lazy=True, load_extensions=False)
		
        analyzer.load_extension('quality_metrics')
        analyzer.load_extension('unit_locations')

        firing_rates.append(metric_list_from_analyzer(analyzer, 'firing_rate'))
        snrs.append(metric_list_from_analyzer(analyzer, 'snr'))
        isi_violations.append(metric_list_from_analyzer(analyzer, 'isi_violations_count'))
        
        unit_ids = analyzer.unit_ids
        y_unit_locations = analyzer.get_extension('unit_locations').get_data()[:,1]
        y_df = pd.DataFrame(index=unit_ids, data=y_unit_locations, columns=[str(start)])
        unit_locations.append(y_df)
...
all_firing_rates = pd.concat(firing_rates, axis=1)
all_firing_rates.to_csv('all_firing_rates_merged.csv')
all_snrs = pd.concat(snrs, axis=1)
all_snrs.to_csv('all_snrs_merged.csv')
all_violations = pd.concat(isi_violations, axis=1)
all_violations.to_csv('all_violations_merged.csv')
all_unit_locations = pd.concat(unit_locations, axis=1)
all_unit_locations.to_csv('all_unit_locations_merged.csv')
